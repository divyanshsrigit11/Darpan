...
namespace Darpan
{
    public partial class Astrology : Page
    {
        private const int AstrologyCategoryId = 17; // ID for 'Astrology' from dbo.NewsCategory
        ...
        private void BindNewsArticles()
        {
            ...
            {
                // Query to fetch news articles for the Astrology category
                string query = @"
                    SELECT
                ...
                {
                    cmd.Parameters.AddWithValue("@NewsCategoryId", AstrologyCategoryId);
