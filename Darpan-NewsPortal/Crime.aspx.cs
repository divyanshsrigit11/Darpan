    ...
    public partial class Crime : Page
    {
        ...
        private void LoadCrimeNews()
        {
            ...
            try
            {
                ...
                {
                    con.Open();
                    // Select news where NewsCategory is 'Crime'
                    string query = @"
                        ...
                        WHERE N.IsActive = 1 AND NC.NewsCategory = 'Crime'
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            rptCrimeNews.DataSource = dt;
                            rptCrimeNews.DataBind();
                            pnlNoContent.Visible = false; // Hide "No Content" message
                        }
                        else
                        {
                            rptCrimeNews.DataSource = null;
                            rptCrimeNews.DataBind();
